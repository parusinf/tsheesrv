...
async def _with_conn(db, db_key: str, cb):
    ...
    ВАЖНО: acquire() и release() в oracledb — синхронные методы.
    """
...
async def get_orgs(db, db_key: str, org_inn: str) -> List[dict]:
    async def body(conn):
        # cursor() — обычный контекстный менеджер (НЕ async with)
